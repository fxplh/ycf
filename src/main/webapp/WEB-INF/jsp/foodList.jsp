...
<div class="header">
    <div class="header_c">
        <a href="https://www.meishij.net/" class="logo pngFix"><img style="height:129px" src="/images/log.jpg"/></a>
    ...
    <div class="main">
        <div class="listnav clearfix" id="listnav">
            <ul class="listnav_ul" id="listnav_ul">
                <c:forEach items="${foodClassList}" var="foodClass" varStatus="status">
                    ...
                    <c:if test="${empty foodClass.childClass}">
                        ><a href="/foodClass/foodList?id=${foodClass.id}" class="link">${foodClass.name}</a></li>
                    </c:if>
                    <c:if test="${!empty foodClass.childClass}">
                        class="current hover" ><h1><a href="/foodClass/foodList?id=${foodClass.id}" class="link">${foodClass.name}</a></h1></li>
                    </c:if>
                </c:forEach>
            </ul>
            <div class="listnav_con clearfix" id="listnav_con_c">
                <dl class="listnav_dl_style1 w990 bb1 clearfix">
                    <c:forEach items="${foodClassList}" var="foodClass">
                        <c:if test="${!empty foodClass.childClass}">
                            <dt>${foodClass.name}
                            <c:forEach items="${foodClass.childClass}" var="childClass">
                                <c:if test="${childId eq childClass.id}">
                                    <dd class="current"><h1><a
                                            href="/foodClass/foodList?id=${childClass.id}">${childClass.name}</a></h1>
                                    </dd>
                                </c:if>
                                <c:if test="${childId ne childClass.id}">
                                    <dd><a href="/foodClass/foodList?id=${childClass.id}">${childClass.name}</a></dd>
                                </c:if>
                            </c:forEach>
                            </dt>
                        </c:if>
                    ...
                    <div class="listtyle1_page">
                        <div class="listtyle1_page_w">
                            <c:if test="${page.countPage gt 1}">
                                <a href="/foodClass/foodList?id=${id}&currPage=${page.currPage-1}" class="prev">上一页</a>
                            </c:if>
                            <c:if test="${page.countPage le 5}">
                                <c:forEach var="index" begin="1" end="${page.countPage}" >
                                    <a href="/foodClass/foodList?id=${id}&currPage=${index}" <c:if test="${page.currPage eq index}">class="current"</c:if>>${index}</a>
                                </c:forEach>
                            </c:if>
                            ...
                            <c:if test="${page.countPage gt page.currPage }">
                                <a href="/foodClass/foodList?id=${id}&currPage=${page.currPage+1}" class="next">下一页</a>
                            </c:if>
                            <span class="gopage">
                                <form action="/foodClass/foodList">
